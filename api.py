...
class PredictAnswer(Resource):

    def load_data(self, question_path, context_path):
        """Load data """
    ...
    def embedding(self, df, infersent_path):
    ...
    def process_data(self, df, dict_emb):
    ...
    def get_columns_from_common_word(self, train):
    ...
    def calculate_distance(self, df):   
        ...
        df["cosine_sim"] = df.apply(self.cosine_sim, axis = 1)
    ...
    def semantic_similarity(self, data):
    ...
    def get_columns_from_root(self, train):
    ...
    def get(self):
        ...
        question = args['question_path']
        context = args['context_path']
        # read and process data
        df = self.load_data(question, context)
        dict_emb = self.embedding(df, 'infersent.pkl')
        df = self.process_data(df, dict_emb)
        ...
        # baseline feature
        df['num_common_word'] = df.apply(self.common_words, axis = 1)
        baseline_feature = self.get_columns_from_common_word(df)
        baseline_feature = baseline_feature.fillna(0)

        # semantic similarity feature
        df = self.calculate_distance(df)
        sem_similar_feature_tmp = self.semantic_similarity(df)
        # fill NA using max
        ...
        # root match feature
        df["root_match_idx"] = df.apply(self.match_roots, axis = 1)
        root_feature = self.get_columns_from_root(df)
...
api.add_resource(PredictAnswer, '/')
